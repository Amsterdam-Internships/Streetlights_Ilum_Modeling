...
class Amsterdam3D:
    ...
    def get_features(self, data):
        ...
        if args.use_intensity:
            try:
                ...
                print(
                    "No intensity values found. Please run prepare.py with --use_intensity flag."
                )
                raise
        if args.use_normals:
            try:
                sub_features.extend(
                    [data["normal_x"], data["normal_y"], data["normal_z"]]
                )
            except KeyError:
                print(
                    "No normal values found. Please run prepare.py with --use_normals flag."
                )
                raise
        if sub_features:
    ...
    def calculate_normals(points_xyz):
        ...
        points = np.stack(
            (points_xyz[:, 0], points_xyz[:, 1], points_xyz[:, 2]), axis=-1
        )
    ...
    def get_raw_features(self, data):
        ...
        if args.use_rgb:
            ...
            features.append(rgb)
        if args.use_intensity:
            intensity = (data.intensity / 65535.0).reshape((-1, 1)).astype(np.float16)
    ...
    def load_sub_sampled_clouds(self, in_folder):
        for i, file_path in enumerate(self.all_files):
            ...
            if self.mode == "test" and args.no_prepare:
                ...
                xyz = np.vstack(
                    (data.x - cfg.x_offset, data.y - cfg.y_offset, data.z)
                ).T.astype(np.float32)
                ...
                sub_xyz, sub_features = DP.grid_sub_sampling(
                    xyz[mask], features[mask], grid_size=cfg.sub_grid_size
                )
                search_tree = KDTree(sub_xyz)
                proj_idx = np.squeeze(
                    search_tree.query(xyz[mask], return_distance=False)
                )
        ...
        def spatially_regular_gen():
            for i in range(num_per_epoch):
                ...
                if self.input_features[split][cloud_idx] is not None:
                    queried_pc_colors = self.input_features[split][cloud_idx][
                        queried_idx
                    ]
                else:
    ...
    def init_input_pipeline_eval(self):
        ...
        self.batch_val_data = self.batch_val_data.prefetch(cfg.val_batch_size)
        iter = tf.data.Iterator.from_structure(
            self.batch_val_data.output_types, self.batch_val_data.output_shapes
        )
        self.flat_inputs = iter.get_next()
        self.val_init_op = iter.make_initializer(self.batch_val_data)
...
def evaluate(in_folder, in_files):
    ...
    if args.snap_folder is not None:
        snap_steps = [
            int(f[:-5].split("-")[-1])
            for f in os.listdir(args.snap_folder)
            if f[-5:] == ".meta"
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--in_folder", metavar="path", action="store", type=str, required=True
    )
