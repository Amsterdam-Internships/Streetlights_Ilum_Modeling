...
class Amsterdam3D:
    ...
    def get_features(self, data):
        ...
        if args.use_rgb:
            try:
                ...
                print('No RGB values found. Please run prepare.py with --use_rgb flag.')
                raise
        if args.use_intensity:
            try:
                sub_features.extend([data['intensity']])
            except KeyError:
                print('No intensity values found. Please run prepare.py with --use_intensity flag.')
                raise
        if args.use_normals:
            try:
                sub_features.extend([data['normal_x'], data['normal_y'], data['normal_z']])
            except KeyError:
                print('No normal values found. Please run prepare.py with --use_normals flag.')
                raise
        if sub_features:
    ...
    def calculate_normals(points_xyz):
        ...
        normals = np.round(np.array(object_pcd.normals), 2)
        return normals
    ...
    def get_raw_features(self, data):
        ...
        if args.use_normals:
            try:
                ...
                print('No normals found. Generating them now...')
    ...
    def load_sub_sampled_clouds(self, in_folder):
        for i, file_path in enumerate(self.all_files):
            ...
            if self.mode == 'test' and args.no_prepare:
                ...
                xyz = (np.vstack((data.x - cfg.x_offset, data.y
                                  - cfg.y_offset, data.z)).T.astype(np.float32))
                ...
                sub_xyz, sub_features = DP.grid_sub_sampling(xyz[mask],
                                                             features[mask],
                ...
                kd_tree_file = join(in_folder, '{:s}_KDTree.pkl'.format(cloud_name))
            ...
            if self.mode == 'evaluate':
                sub_labels = data['label'].squeeze()
                self.input_labels[cloud_split] += [sub_labels]
            if self.mode == 'test' and not args.no_prepare:
    ...
    def get_batch_gen(self, split):
        ...
        def spatially_regular_gen():
            for i in range(num_per_epoch):
                ...
                dists = np.sum(np.square((points[queried_idx] - pick_point).astype(np.float32)),
                               axis=1)
                delta = np.square(1 - dists / np.max(dists))
                self.possibility[split][cloud_idx][queried_idx] += delta
                self.min_possibility[split][cloud_idx] = float(np.min(self.
                                                                      possibility[split]
                                                                      [cloud_idx]))
                if len(points) < cfg.num_points:
                    queried_pc_xyz, queried_pc_colors,
                    queried_idx, queried_pc_labels = DP.data_aug(
    ...
    def get_tf_mapping2():
        def tf_map(batch_xyz, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx):
            ...
            for i in range(cfg.num_layers):
                neighbour_idx = tf.py_function(DP.knn_search, [batch_xyz, batch_xyz,
                                                               cfg.k_n], tf.int32)
    ...
    def init_input_pipeline_train(self):
        ...
        cfg.ignored_label_inds = [self.label_to_idx[ign_label] for ign_label in
                                  self.ignored_labels]
        gen_function, gen_types, gen_shapes = self.get_batch_gen('training')
        gen_function_val, _, _ = self.get_batch_gen('validation')
        self.train_data = tf.data.Dataset.from_generator(gen_function, gen_types,
                                                         gen_shapes)
        self.val_data = tf.data.Dataset.from_generator(gen_function_val, gen_types,
                                                       gen_shapes)
    ...
    def init_input_pipeline_eval(self):
        ...
        self.batch_val_data = self.batch_val_data.map(map_func=map_func)
        self.batch_val_data = self.batch_val_data.prefetch(cfg.val_batch_size)
        iter = tf.data.Iterator.from_structure(self.batch_val_data.output_types,
    ...
    def init_input_pipeline_test(self):
        ...
        self.test_data = tf.data.Dataset.from_generator(gen_function_test, gen_types,
                                                        gen_shapes)
...
def evaluate(in_folder, in_files):
    ...
    if args.snap_folder is not None:
        snap_steps = [int(f[:-5].split('-')[-1]) for f in os.listdir(args.snap_folder)
                      if f[-5:] == '.meta']
        chosen_step = np.sort(snap_steps)[-1]
        chosen_snap = os.path.join(args.snap_folder, 'snap-{:d}'.format(chosen_step))
    else:
        logs = np.sort([os.path.join('results', f) for f in os.listdir('results')
                        if f.startswith('Log')])
        chosen_folder = logs[-1]
        snap_path = join(chosen_folder, 'snapshots')
        snap_steps = [int(f[:-5].split('-')[-1]) for f in os.listdir(snap_path)
                      if f[-5:] == '.meta']
        chosen_step = np.sort(snap_steps)[-1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--in_folder', metavar='path', action='store', type=str, required=True)
    parser.add_argument('--out_folder', metavar='path', action='store', type=str)
    parser.add_argument('--snap_folder', metavar='path', action='store', type=str)
    parser.add_argument('--resume', action='store_true')
    parser.add_argument('--resume_path', metavar='path', action='store', type=str)
