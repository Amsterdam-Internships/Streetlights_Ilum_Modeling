...
def read_and_filter_points(filepath, label_value=2):
    with laspy.open(filepath) as file:
        ...
        try:
            labels = lidar[
                "label"
            ]  # Replace 'label' with the actual field name if different
        except KeyError:
            raise KeyError(
                "The specified field 'label' does not exist in the LAS file."
            )
        # Applying filter
...
def main():
    ...
    parser.add_argument(
        "--label", type=int, default=2, help="Label value to filter points (default: 2)"
    )
