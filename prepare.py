...
def run():
    ...
    if args.resume:
        ...
        print(done_tiles)
        files = [
            f for f in in_files if utils.get_tilecode_from_filename(f) not in done_tiles
        ]
    else:
        files = in_files

    # Create the sub folder
    out_folder = join(args.out_folder, f"input_{cfg.sub_grid_size:.3f}")
    ...
    for f in files_tqdm:
        ...
        if args.mode == "train":
            ...
            sub_xyz, sub_features, sub_labels = DP.grid_sub_sampling(
                xyz[mask], features[mask], labels_prepared, grid_size=cfg.sub_grid_size
            )
            ...
            if args.save_ply:
                ...
                write_ply(
                    sub_ply_file, [sub_xyz, sub_features, sub_labels], ply_content
                )
            else:
