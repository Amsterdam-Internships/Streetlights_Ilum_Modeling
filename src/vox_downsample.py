...
def downsample_point_cloud(points, color=None, intensity=None, normals=None,
                           segment=None, voxel_size=0.05):
    """
...
def save_laz_file(points, colors=None, intensity=None, normals=None, segments=None,
                  output_path=None):
    """
    ...
    if segments is not None:
        las.add_extra_dim(laspy.ExtraBytesParams(name="label", type=np.uint8,
...
def process_folder(input_folder, output_folder, voxel_size=0.05):
    ...
    for file_name in os.listdir(input_folder):
        if file_name.endswith('.laz'):
            ...
            (downsampled_points, downsampled_colors, downsampled_intensity,
             downsampled_normals, downsampled_segment) = downsample_point_cloud(
                points, color=color, intensity=intensity, normals=normals,
                segment=segment, voxel_size=voxel_size)
            save_laz_file(downsampled_points, colors=downsampled_colors,
                          intensity=downsampled_intensity, normals=downsampled_normals,
                          segments=downsampled_segment, output_path=output_path)
