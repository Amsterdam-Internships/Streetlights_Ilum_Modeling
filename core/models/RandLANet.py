...
# log mean of metric and metric per class
def log_metric_per_class(metric_name, mean_metric, class_metrics, class_names, results_dict, log_file):
    log_out('{} per class'.format(metric_name), log_file)
    s = '{:5.2f} | '.format(100 * mean_metric)
    for class_name, class_metric in zip(class_names, class_metrics):
        if np.isnan(class_metric):
            IoU = 0
        s += '{:5.2f} '.format(100 * class_metric)
        results_dict['{} '.format(metric_name) + class_name] = class_metric
    log_out('-' * len(s), log_file)
    log_out(s, log_file)
    log_out('-' * len(s) + '\n', log_file)

# log metric per class as list
def log_metric_per_class_as_list(metric_name, class_metrics, log_file):
    log_out('{} List'.format(metric_name), log_file)
    s = '['
    for i in class_metrics:
        s += '{:5.2f}, '.format(100 * i)
    s = s[:-2]
    s += ']'
    log_out('{}\n'.format(s), log_file)

def log_confusion_matrix_as_list(matrix_name, conf_matrix, log_file):
    log_out('{} confusion matrix as list:'.format(matrix_name), log_file)
    m = conf_matrix
    result = '['
    for i in range(len(m)):
        result = result + '['
        for j in range((len(m[i]))):
            if matrix_name == 'Precision':
                result = result + '{:0.3f}'.format(m[i][j]/sum(np.transpose(m)[j]))
            elif matrix_name == 'Recall':
                result = result + '{:0.3f}'.format(m[i][j]/sum(m[i]))
            elif matrix_name == 'Summed':
        ...
        if i < len(m) - 1:
            ...
            result = result + ']]\n'
    log_out('{}'.format(result), log_file)
...
class Network:
    def __init__(self, dataset, config, resume=False, resume_path=None):
        ...
        self.config = config
        features_used = dataset.features_used
        # Path of the result folder
        if self.config.saving:
            if self.config.saving_path is None:
                if not os.path.isdir('results/{}/'.format(features_used)):
                    os.makedirs('results/{}/'.format(features_used))
                if resume:
                    self.saving_path = join('results/{}/'.format(features_used), sorted(listdir('results/'))[-1])
                else: 
                    self.saving_path = time.strftime('results/{}/Log_%Y-%m-%d_%H-%M-%S'.format(features_used), time.gmtime())
            else:
        ...
        with tf.variable_scope('inputs'):
            ...
            self.Log_file = open('train_logs/log_train_' + features_used + log_time + '.txt', 'w+')
        ...
        if resume:
            if resume_path is None:
                ...
                resume_path = join('results/', sorted(listdir('results/'))[-1] + '/')
            ...
            self.training_epoch = int((best_step-1) / self.config.train_steps)
    ...
    def evaluate(self, dataset):
        ...
        # calculate iou, precision and recall per class
        ...
        # calculate accuracy and mean of iou, precision and recall
        ...
        # log iou, precision and recall per class
        log_metric_per_class('IoU', mean_iou, iou_list, class_names, results_dict, self.Log_file)
        log_metric_per_class('Precision', mean_precision, precision_list, class_names, results_dict, self.Log_file)
        log_metric_per_class('Recall', mean_recall, recall_list, class_names, results_dict, self.Log_file)

        # log iou, precision, recall and confusion matrices as lists if model is best so far
        if mean_iou > np.max(self.miou_list):

            # log iou, precision and recall per class as list
            log_metric_per_class_as_list('IoU', iou_list, self.Log_file)
            log_metric_per_class_as_list('Precision', precision_list, self.Log_file)
            log_metric_per_class_as_list('Recall', recall_list, self.Log_file)

            # log precision, recall and summed confusion matrix as list
            log_confusion_matrix_as_list('Precision', conf_matrix, self.Log_file)
            log_confusion_matrix_as_list('Recall', conf_matrix, self.Log_file)
            log_confusion_matrix_as_list('Summed', conf_matrix, self.Log_file)
        
        # log iou, precision and recall for tensorboard
